...
Syntax: `getcoins.py [-h|--help] [-c|--cmd=<koyotecoin-cli path>] [-f|--faucet=<faucet URL>] [-a|--addr=<signet bech32 address>] [-p|--password=<faucet password>] [--] [<koyotecoin-cli args>]`

* `--cmd` lets you customize the koyotecoin-cli path. By default it will look for it in the PATH
* `--faucet` lets you specify which faucet to use; the faucet is assumed to be compatible with https://github.com/kallewoof/koyotecoin-faucet
* `--addr` lets you specify a Signet address; by default, the address must be a bech32 address. This and `--cmd` above complement each other (i.e. you do not need `koyotecoin-cli` if you use `--addr`)
...
To mine the first block in your custom chain, you can run:
    ...
    CLI="./koyotecoin-cli -conf=mysignet.conf"
...
The process generate follows internally is to get a block template, convert that into a PSKT, sign the PSKT, move the signature from the signed PSKT into the block template's coinbase, grind proof of work for the block, and then submit the block to the network.

These steps can instead be done explicitly:

    $CLI -signet getblocktemplate '{"rules": ["signet","segwit"]}' |
      $MINER --cli="$CLI" genpskt --address="$ADDR" |
      $CLI -signet -stdin walletprocesspskt |
      jq -r .pskt |
      $MINER --cli="$CLI" solvepskt --grind-cmd="$GRIND" |
