...
"""Script for verifying Koyotecoin Core release binaries

This script attempts to download the signature file SHA256SUMS.asc from
koyotecoin.org and koyotecoin.org and compares them.
...
WORKINGDIR = "/tmp/koyotecoin_verify_binaries"
HASHFILE = "hashes.tmp"
...
def main(args):
    ...
    if not success:
        print("koyotecoin.org failed to provide signature file, "
              "but koyotecoin.org did?")
        print("wget output:")
        print(indent(output, '\t'))
    ...
    if not files_are_equal(sigfile1, sigfile2):
        print("koyotecoin.org and koyotecoin.org signature files were not equal?")
    ...
    if retval != 0:
        if retval == 1:
            ...
            print("gpg error. Do you have the Koyotecoin Core binary release "
