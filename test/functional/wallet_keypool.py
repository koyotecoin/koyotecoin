...
class KeyPoolTest(KoyotecoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_raises_rpc_error(-4, "Transaction needs a change address, but we can't generate it.", w2.walletcreatefundedpskt, inputs=[], outputs=[{addr.pop(): 0.00005000}], options={"subtractFeeFromOutputs": [0], "feeRate": 0.00010})

        # creating a 10,000 howl transaction without change, with a manual input, should still be possible
        res = w2.walletcreatefundedpskt(inputs=w2.listunspent(), outputs=[{destination: 0.00010000}], options={"subtractFeeFromOutputs": [0], "feeRate": 0.00010})
        assert_equal("pskt" in res, True)

        # creating a 10,000 howl transaction without change should still be possible
        res = w2.walletcreatefundedpskt(inputs=[], outputs=[{destination: 0.00010000}], options={"subtractFeeFromOutputs": [0], "feeRate": 0.00010})
        assert_equal("pskt" in res, True)
        # should work without subtractFeeFromOutputs if the exact fee is subtracted from the amount
        res = w2.walletcreatefundedpskt(inputs=[], outputs=[{destination: 0.00008900}], options={"feeRate": 0.00010})
        assert_equal("pskt" in res, True)

        # dust change should be removed
        res = w2.walletcreatefundedpskt(inputs=[], outputs=[{destination: 0.00008800}], options={"feeRate": 0.00010})
        assert_equal("pskt" in res, True)

        # create a transaction without change at the maximum fee rate, such that the output is still spendable:
        res = w2.walletcreatefundedpskt(inputs=[], outputs=[{destination: 0.00010000}], options={"subtractFeeFromOutputs": [0], "feeRate": 0.0008823})
        assert_equal("pskt" in res, True)
        assert_equal(res["fee"], Decimal("0.00009706"))

        # creating a 10,000 howl transaction with a manual change address should be possible
        res = w2.walletcreatefundedpskt(inputs=[], outputs=[{destination: 0.00010000}], options={"subtractFeeFromOutputs": [0], "feeRate": 0.00010, "changeAddress": addr.pop()})
        assert_equal("pskt" in res, True)
