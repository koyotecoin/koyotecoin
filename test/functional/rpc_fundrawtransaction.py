...
class RawTransactionsTest(KoyotecoinTestFramework):
    ...
    def unlock_utxos(self, wallet):
        ...
        if self.watchonly_txid is not None and self.watchonly_vout is not None:
            to_keep.append({"txid": self.watchonly_txid,
    ...
    def test_change_position(self):
        ...
        self.nodes[0].sendtoaddress(self.nodes[3].get_wallet_rpc(
            self.default_wallet_name).getnewaddress(), self.watchonly_amount / 10)
    ...
    def test_simple_two_outputs(self):
        ...
        inputs = []
        outputs = {self.nodes[0].getnewaddress(
        ): 2.6, self.nodes[1].getnewaddress(): 2.5}
        rawtx = self.nodes[2].createrawtransaction(inputs, outputs)
        dec_tx = self.nodes[2].decoderawtransaction(rawtx)
    ...
    def test_no_change(self):
        ...
        inputs = [{'txid': utx['txid'], 'vout': utx['vout']}]
        outputs = {self.nodes[0].getnewaddress(): Decimal(
            5.0) - self.test_no_change_fee - self.fee_tolerance}
        rawtx = self.nodes[2].createrawtransaction(inputs, outputs)
        dec_tx = self.nodes[2].decoderawtransaction(rawtx)
        assert_equal(utx['txid'], dec_tx['vin'][0]['txid'])
    ...
    def test_invalid_option(self):
        ...
        assert_raises_rpc_error(-3, "Unexpected key reserveChangeKey",
                                lambda: self.nodes[2].fundrawtransaction(hexstring=rawtx, options={'reserveChangeKey': True}))
    ...
    def test_invalid_change_address(self):
        ...
        assert_raises_rpc_error(-5, "Change address must be a valid koyotecoin address",
    ...
    def test_valid_change_address(self):
        ...
        assert_raises_rpc_error(-8, "changePosition out of bounds",
                                self.nodes[2].fundrawtransaction, rawtx, {'changeAddress': change, 'changePosition': 2})
    ...
    def test_change_type(self):
        ...
        outputs = {self.nodes[0].getnewaddress(): Decimal(4.0)}
        rawtx = self.nodes[2].createrawtransaction(inputs, outputs)
        assert_raises_rpc_error(-1, "JSON value of type null is not of expected type string",
                                self.nodes[2].fundrawtransaction, rawtx, {'change_type': None})
        assert_raises_rpc_error(-5, "Unknown change type ''",
                                self.nodes[2].fundrawtransaction, rawtx, {'change_type': ''})
    ...
    def test_coin_selection(self):
        ...
        assert_raises_rpc_error(-4, "Insufficient funds",
                                self.nodes[2].fundrawtransaction, rawtx, {"add_inputs": False})
    ...
    def test_two_vin_two_vout(self):
        ...
        inputs = [{'txid': utx['txid'], 'vout': utx['vout']},
                  {'txid': utx2['txid'], 'vout': utx2['vout']}]
        outputs = {self.nodes[0].getnewaddress(
        ): 6.0, self.nodes[0].getnewaddress(): 1.0}
        rawtx = self.nodes[2].createrawtransaction(inputs, outputs)
        dec_tx = self.nodes[2].decoderawtransaction(rawtx)
        assert_equal(utx['txid'], dec_tx['vin'][0]['txid'])
    ...
    def test_invalid_input(self):
        ...
        outputs = {self.nodes[0].getnewaddress(): 1.0}
        rawtx = self.nodes[2].createrawtransaction(inputs, outputs)
        assert_raises_rpc_error(-4, "Unable to find UTXO for external input",
    ...
    def test_spend_2of2(self):
        ...
        self.log.info("Test fundpskt spending 2-of-2 multisig")
        ...
        inputs = []
        outputs = {self.nodes[1].getnewaddress(): 1.1}
        funded_pskt = wmulti.walletcreatefundedpskt(inputs=inputs, outputs=outputs, options={
                                                    'changeAddress': w2.getrawchangeaddress()})['pskt']

        signed_pskt = w2.walletprocesspskt(funded_pskt)
        final_pskt = w2.finalizepskt(signed_pskt['pskt'])
        self.nodes[2].sendrawtransaction(final_pskt['hex'])
        self.generate(self.nodes[2], 1)
    ...
    def test_locked_wallet(self):
        ...
        value = sum(inp["amount"] for inp in inputs) - \
            Decimal("0.00000500")  # Pay a 500 howl fee
    ...
    def test_many_inputs_fee(self):
        ...
        inputs = []
        outputs = {self.nodes[0].getnewaddress(
        ): 0.15, self.nodes[0].getnewaddress(): 0.04}
        rawtx = self.nodes[1].createrawtransaction(inputs, outputs)
        fundedTx = self.nodes[1].fundrawtransaction(rawtx)
    ...
    def test_all_watched_funds(self):
        ...
        assert_equal(result["fee"] + res_dec["vout"][0]
                     ["value"], self.watchonly_amount)
    ...
    def test_option_feerate(self):
        self.log.info(
            "Test fundrawtxn with explicit fee rates (fee_rate howl/vB and feeRate KYC/kvB)")
        node = self.nodes[3]
        ...
        kyc_kvb_to_howl_vb = 100000  # (1e5)
        result1 = node.fundrawtransaction(
            rawtx, {"fee_rate": str(2 * kyc_kvb_to_howl_vb * self.min_relay_tx_fee)})
        result2 = node.fundrawtransaction(
            rawtx, {"feeRate": 2 * self.min_relay_tx_fee})
        result3 = node.fundrawtransaction(
            rawtx, {"fee_rate": 10 * kyc_kvb_to_howl_vb * self.min_relay_tx_fee})
        result4 = node.fundrawtransaction(
            rawtx, {"feeRate": str(10 * self.min_relay_tx_fee)})

        result_fee_rate = result['fee'] * 1000 / count_bytes(result['hex'])
        assert_fee_amount(result1['fee'], count_bytes(
            result1['hex']), 2 * result_fee_rate)
        assert_fee_amount(result2['fee'], count_bytes(
            result2['hex']), 2 * result_fee_rate)
        assert_fee_amount(result3['fee'], count_bytes(
            result3['hex']), 10 * result_fee_rate)
        assert_fee_amount(result4['fee'], count_bytes(
            result4['hex']), 10 * result_fee_rate)

        # Test that funding non-standard "zero-fee" transactions is valid.
        for param, zero_value in product(["fee_rate", "feeRate"], [0, 0.000, 0.00000000, "0", "0.000", "0.00000000"]):
            assert_equal(self.nodes[3].fundrawtransaction(
                rawtx, {param: zero_value})["fee"], 0)

        # With no arguments passed, expect fee of 141 howloshis.
        assert_approx(node.fundrawtransaction(rawtx)[
                      "fee"], vexp=0.00000141, vspan=0.00000001)
        ...
        for param, value in {("fee_rate", 100000), ("feeRate", 1.000)}:
            ...
            for invalid_value in ["", 0.000000001, 1e-09, 1.111111111, 1111111111111111, "31.999999999999999999999"]:
                assert_raises_rpc_error(-3, "Invalid amount", node.fundrawtransaction, rawtx, {
        ...
        self.log.info(
            "Test min fee rate checks are bypassed with fundrawtxn, e.g. a fee_rate under 1 howl/vB is allowed")
    ...
    def test_option_subtract_fee_from_outputs(self):
        ...
        # Test subtract fee from outputs with feeRate (KYC/kvB)
        ...
        dec_tx = [self.nodes[3].decoderawtransaction(
        ...
        # Test subtract fee from outputs with fee_rate (howl/vB)
        kyc_kvb_to_howl_vb = 100000  # (1e5)
        result = [self.nodes[3].fundrawtransaction(rawtx),  # uses self.min_relay_tx_fee (set by settxfee)
                  self.nodes[3].fundrawtransaction(
                      rawtx, {"subtractFeeFromOutputs": []}),  # empty subtraction list
                  ...
                  self.nodes[3].fundrawtransaction(
                      rawtx, {"subtractFeeFromOutputs": [0]}),
                  self.nodes[3].fundrawtransaction(
                      rawtx, {"fee_rate": 2 * kyc_kvb_to_howl_vb * self.min_relay_tx_fee}),
                  self.nodes[3].fundrawtransaction(rawtx, {"fee_rate": 2 * kyc_kvb_to_howl_vb * self.min_relay_tx_fee, "subtractFeeFromOutputs": [0]}), ]
        dec_tx = [self.nodes[3].decoderawtransaction(
            tx_['hex']) for tx_ in result]
        output = [d['vout'][1 - r['changepos']]['value']
                  for d, r in zip(dec_tx, result)]
        ...
        inputs = []
        outputs = {self.nodes[2].getnewaddress(
        ): value for value in (1.0, 1.1, 1.2, 1.3)}
        rawtx = self.nodes[3].createrawtransaction(inputs, outputs)
    ...
    def test_transaction_too_large(self):
        ...
        # Make 1500 0.1 KYC outputs. The amount that we target for funding is in
    ...
    def test_external_inputs(self):
        ...
        assert_raises_rpc_error(-4, "Insufficient funds",
                                wallet.fundrawtransaction, raw_tx)

        # Error conditions
        assert_raises_rpc_error(-5, "'not a pubkey' is not hex", wallet.fundrawtransaction,
                                raw_tx, {"solving_data": {"pubkeys": ["not a pubkey"]}})
        assert_raises_rpc_error(-5, "'01234567890a0b0c0d0e0f' is not a valid public key",
                                wallet.fundrawtransaction, raw_tx, {"solving_data": {"pubkeys": ["01234567890a0b0c0d0e0f"]}})
        assert_raises_rpc_error(-5, "'not a script' is not hex", wallet.fundrawtransaction,
                                raw_tx, {"solving_data": {"scripts": ["not a script"]}})
        assert_raises_rpc_error(-8, "Unable to parse descriptor 'not a descriptor'",
                                wallet.fundrawtransaction, raw_tx, {"solving_data": {"descriptors": ["not a descriptor"]}})
        assert_raises_rpc_error(-8, "Invalid parameter, missing vout key",
                                wallet.fundrawtransaction, raw_tx, {"input_weights": [{"txid": ext_utxo["txid"]}]})
        assert_raises_rpc_error(-8, "Invalid parameter, vout cannot be negative", wallet.fundrawtransaction,
                                raw_tx, {"input_weights": [{"txid": ext_utxo["txid"], "vout": -1}]})
        assert_raises_rpc_error(-8, "Invalid parameter, missing weight key", wallet.fundrawtransaction,
                                raw_tx, {"input_weights": [{"txid": ext_utxo["txid"], "vout": ext_utxo["vout"]}]})
        assert_raises_rpc_error(-8, "Invalid parameter, weight cannot be less than 165", wallet.fundrawtransaction,
                                raw_tx, {"input_weights": [{"txid": ext_utxo["txid"], "vout": ext_utxo["vout"], "weight": 164}]})
        assert_raises_rpc_error(-8, "Invalid parameter, weight cannot be less than 165", wallet.fundrawtransaction,
                                raw_tx, {"input_weights": [{"txid": ext_utxo["txid"], "vout": ext_utxo["vout"], "weight": -1}]})
        assert_raises_rpc_error(-8, "Invalid parameter, weight cannot be greater than", wallet.fundrawtransaction, raw_tx, {
                                "input_weights": [{"txid": ext_utxo["txid"], "vout": ext_utxo["vout"], "weight": 400001}]})

        # But funding should work when the solving data is provided
        funded_tx = wallet.fundrawtransaction(raw_tx, {"solving_data": {"pubkeys": [
                                              addr_info['pubkey']], "scripts": [addr_info["embedded"]["scriptPubKey"]]}})
        ...
        funded_tx = wallet.fundrawtransaction(raw_tx, {"input_weights": [
                                              {"txid": ext_utxo["txid"], "vout": ext_utxo["vout"], "weight": input_weight}]})
        signed_tx = wallet.signrawtransactionwithwallet(funded_tx["hex"])
        signed_tx = self.nodes[0].signrawtransactionwithwallet(
            signed_tx["hex"])
        ...
        funded_tx2 = wallet.fundrawtransaction(raw_tx, {"input_weights": [
                                               {"txid": ext_utxo["txid"], "vout": ext_utxo["vout"], "weight": low_input_weight}]})
        assert_greater_than(funded_tx["fee"], funded_tx2["fee"])
        # Increasing the weight should have a higher fee
        funded_tx2 = wallet.fundrawtransaction(raw_tx, {"input_weights": [
                                               {"txid": ext_utxo["txid"], "vout": ext_utxo["vout"], "weight": high_input_weight}]})
        assert_greater_than(funded_tx2["fee"], funded_tx["fee"])
        # The provided weight should override the calculated weight when solving data is provided
        funded_tx3 = wallet.fundrawtransaction(raw_tx, {"solving_data": {"descriptors": [desc]}, "input_weights": [
                                               {"txid": ext_utxo["txid"], "vout": ext_utxo["vout"], "weight": high_input_weight}]})
        assert_equal(funded_tx2["fee"], funded_tx3["fee"])
        # The feerate should be met
        funded_tx4 = wallet.fundrawtransaction(raw_tx, {"input_weights": [
                                               {"txid": ext_utxo["txid"], "vout": ext_utxo["vout"], "weight": high_input_weight}], "fee_rate": 10})
        input_add_weight = high_input_weight - (41 * 4)
        tx4_weight = wallet.decoderawtransaction(funded_tx4["hex"])[
            "weight"] + input_add_weight
        ...
        funded_tx = wallet.fundrawtransaction(raw_tx, {"input_weights": [
                                              {"txid": ext_utxo["txid"], "vout": ext_utxo["vout"], "weight": 255}]})
        funded_tx = wallet.fundrawtransaction(raw_tx, {"input_weights": [
                                              {"txid": ext_utxo["txid"], "vout": ext_utxo["vout"], "weight": 65539}]})
    ...
    def test_add_inputs_default_value(self):
        ...
        # Create and fund the wallet with 5 KYC
        ...
        # Fund wallet with 2 outputs, 5 KYC each.
        ...
        assert_raises_rpc_error(-4, "Insufficient funds",
                                wallet.send, outputs=[{addr1: 8}], options=options)
        ...
        decoded_pskt_inputs = self.nodes[0].decodepskt(tx["pskt"])['tx']['vin']
        assert_equal(len(decoded_pskt_inputs), 2)
        for input in decoded_pskt_inputs:
        ...
        # Case (1), 'walletcreatefundedpskt' command
        # Default add_inputs value with no preset inputs (add_inputs=true)
        inputs = []
        outputs = {self.nodes[1].getnewaddress(): 8}
        assert "pskt" in wallet.walletcreatefundedpskt(
            inputs=inputs, outputs=outputs)

        # Case (2), 'walletcreatefundedpskt' command
        ...
        outputs = {self.nodes[1].getnewaddress(): 8}
        assert_raises_rpc_error(-4, "Insufficient funds",
                                wallet.walletcreatefundedpskt, inputs=inputs, outputs=outputs)
        ...
        pskt_tx = wallet.walletcreatefundedpskt(
            outputs=[{addr1: 8}], inputs=inputs, options=options)
        # Check that only the preset inputs were added to the tx
        decoded_pskt_inputs = self.nodes[0].decodepskt(pskt_tx["pskt"])[
            'tx']['vin']
        assert_equal(len(decoded_pskt_inputs), 2)
        for input in decoded_pskt_inputs:
            assert_equal(input["txid"], source_tx["txid"])

        # Case (5), 'walletcreatefundedpskt' command
    ...
    def test_preset_inputs_selection(self):
        ...
        # Create and fund the wallet with 4 UTXO of 5 KYC each (20 KYC total)
        ...
        # Attempt to send 29 KYC from a wallet that only has 20 KYC. The wallet should exclude
        # the preset inputs from the pool of available coins, realize that there is not enough
        # money to fund the 29 KYC payment, and fail with "Insufficient funds".
        #
        # Even with SFFO, the wallet can only afford to send 20 KYC.
        # If the wallet does not properly exclude preset inputs from the pool of available coins
        # prior to coin selection, it may create a transaction that does not fund the full payment
        # amount or, through SFFO, incorrectly reduce the recipient's amount by the difference
        # between the original target and the wrongly counted inputs (in this case 9 KYC)
        # so that the recipient's amount is no longer equal to the user's selected target of 29 KYC.

        # First case, use 'subtract_fee_from_outputs = true'
        assert_raises_rpc_error(-4, "Insufficient funds", wallet.send, outputs=[
                                {wallet.getnewaddress(address_type="bech32"): 29}], options=options)
    ...
    def test_weight_calculation(self):
        ...
        rawtx = wallet.createrawtransaction([{'txid': txid, 'vout': vout}], [
        ...
        rawtx = wallet.createrawtransaction([{'txid': txid, 'vout': ext_vout}], [
    ...
    def test_22670(self):
        ...
        # that this bug is not detected because the transaction fee must be at least 0.01 KYC (the minimum change value).
        # Otherwise the targeted minimum change value will be enough to cover the transaction fees that were not
        # being accounted for. So the minimum relay fee is set to 0.1 KYC/kvB in this test.
        ...
        upper_bound = Decimal("2.0") - cost_of_change - \
            overhead_fees - Decimal("0.01")
    ...
    def test_feerate_rounding(self):
        ...
        # At a feerate of 1.85 howl/vb, the input will need a fee of 125.8 howls and the rest 77.7 howls
        # The entire tx fee should be 203.5 howls.
        ...
        # To test this does not happen, we subtract 202 howls from the input value. If working correctly, this should
        # fail with insufficient funds rather than koyotecoind asserting.
        rawtx = w.createrawtransaction(inputs=[], outputs=[
                                       {self.nodes[0].getnewaddress(address_type="bech32"): 1 - 0.00000202}])
        assert_raises_rpc_error(-4, "Insufficient funds",
                                w.fundrawtransaction, rawtx, {"fee_rate": 1.85})
