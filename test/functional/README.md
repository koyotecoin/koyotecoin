...
- Use a module-level docstring to describe what the test is testing, and how it
  is testing it.
- When subclassing the KoyotecoinTestFramework, place overrides for the
...
- Set the `self.setup_clean_chain` variable in `set_test_params()` to `True` to
  ...
  mature block subsidies (25x50=1250 KYC) in its wallet. Using them is much more
...
with the koyotecoind(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a koyotecoind.  `P2PInterface`
contains the higher level logic for processing P2P payloads and connecting to
the Koyotecoin Core node application logic. For custom behaviour, subclass the
...
The [`TestShell`](test-shell.md) class exposes the KoyotecoinTestFramework
...
Taken from the [python-koyotecoinrpc repository](https://github.com/jgarzik/python-koyotecoinrpc).
...
Test objects for interacting with a koyotecoind node over the p2p interface.

#### [script.py](test_framework/script.py)
Utilities for manipulating transaction scripts (originally from python-koyotecoinlib)
