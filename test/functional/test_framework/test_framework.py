...
TMPDIR_PREFIX = "koyotecoin_func_test_"
...
class KoyotecoinTestMetaClass(type):
    """Metaclass for KoyotecoinTestFramework.

    Ensures that any attempt to register a subclass of `KoyotecoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'KoyotecoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("KoyotecoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("KoyotecoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class KoyotecoinTestFramework(metaclass=KoyotecoinTestMetaClass):
    """Base class for a koyotecoin test script.

    Individual koyotecoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave koyotecoinds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop koyotecoinds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use koyotecoin-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_koyotecoind = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "koyotecoind" + config["environment"]["EXEEXT"],
        )
        fname_koyotecoincli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "koyotecoin-cli" + config["environment"]["EXEEXT"],
        )
        fname_koyotecoinutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "koyotecoin-util" + config["environment"]["EXEEXT"],
        )
        self.options.koyotecoind = os.getenv("KOYOTECOIND", default=fname_koyotecoind)
        self.options.koyotecoincli = os.getenv("KOYOTECOINCLI", default=fname_koyotecoincli)
        self.options.koyotecoinutil = os.getenv("KOYOTECOINUTIL", default=fname_koyotecoinutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: koyotecoinds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("KoyotecoinRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'koyotecoin-cli', self.options.koyotecoincli) for v in versions]
        ...
        for i in range(num_nodes):
            test_node_i = TestNode(
                ...
                koyotecoind=binary[i],
                koyotecoin_cli=binary_cli[i],
                version=versions[i],
            ...
            if not test_node_i.version_is_at_least(170000):
                ...
                conf_file = test_node_i.koyotecoinconf
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a koyotecoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple koyotecoind test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as koyotecoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    koyotecoind=self.options.koyotecoind,
                    koyotecoin_cli=self.options.koyotecoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in koyotecoin.conf
    ...
    def skip_if_no_koyotecoind_tracepoints(self):
        """Skip the running test if koyotecoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("koyotecoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_koyotecoind_zmq(self):
        """Skip the running test if koyotecoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("koyotecoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if koyotecoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("koyotecoin-wallet has not been compiled")

    def skip_if_no_koyotecoin_util(self):
        """Skip the running test if koyotecoin-util has not been compiled."""
        if not self.is_koyotecoin_util_compiled():
            raise SkipTest("koyotecoin-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if koyotecoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("koyotecoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether koyotecoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether koyotecoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_koyotecoin_util_compiled(self):
        """Checks whether koyotecoin-util was compiled."""
        return self.config["components"].getboolean("ENABLE_KOYOTECOIN_UTIL")
