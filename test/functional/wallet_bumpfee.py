...
class BumpFeeTest(KoyotecoinTestFramework):
    ...
    def run_test(self):
        ...
        test_watchonly_pskt(self, peer_node, rbf_node, dest_address)
    ...
    def test_invalid_parameters(self, rbf_node, peer_node, dest_address):
        ...
        for key in ["totalFee", "feeRate"]:
            assert_raises_rpc_error(-3, "Unexpected key {}".format(key),
                                    rbf_node.bumpfee, rbfid, {key: NORMAL})

        # Bumping to just above minrelay should fail to increase the total fee enough.
        assert_raises_rpc_error(-8, "Insufficient total fee 0.00000141",
        ...
        for zero_value in [0, 0.000, 0.00000000, "0", "0.000", "0.00000000"]:
            assert_raises_rpc_error(-8, msg, rbf_node.bumpfee,
                                    rbfid, {"fee_rate": zero_value})
        msg = "Invalid amount"
        # Test fee_rate values that don't pass fixed-point parsing checks.
        for invalid_value in ["", 0.000000001, 1e-09, 1.111111111, 1111111111111111, "31.999999999999999999999"]:
            assert_raises_rpc_error(-3, msg, rbf_node.bumpfee,
                                    rbfid, {"fee_rate": invalid_value})
        ...
        assert_raises_rpc_error(-3, "Amount out of range",
                                rbf_node.bumpfee, rbfid, {"fee_rate": -1})
        # Test type error.
        for value in [{"foo": "bar"}, True]:
            assert_raises_rpc_error(-3, "Amount is not a number or string",
                                    rbf_node.bumpfee, rbfid, {"fee_rate": value})
        ...
        for mode in ["foo", Decimal("3.1415"), "howl/B", "KYC/kB"]:
...
def test_simple_bumpfee_succeeds(self, mode, rbf_node, peer_node, dest_address):
    ...
    assert rbfid in rbf_node.getrawmempool() and rbfid in peer_node.getrawmempool()
    if mode == "fee_rate":
        bumped_pskt = rbf_node.psktbumpfee(rbfid, {"fee_rate": str(NORMAL)})
        bumped_tx = rbf_node.bumpfee(rbfid, {"fee_rate": NORMAL})
    else:
        bumped_pskt = rbf_node.psktbumpfee(rbfid)
        bumped_tx = rbf_node.bumpfee(rbfid)
    assert_equal(bumped_tx["errors"], [])
    assert bumped_tx["fee"] > -rbftx["fee"]
    assert_equal(bumped_tx["origfee"], -rbftx["fee"])
    assert "pskt" not in bumped_tx
    assert_equal(bumped_pskt["errors"], [])
    assert bumped_pskt["fee"] > -rbftx["fee"]
    assert_equal(bumped_pskt["origfee"], -rbftx["fee"])
    assert "pskt" in bumped_pskt
...
def test_segwit_bumpfee_succeeds(self, rbf_node, dest_address):
    ...
    segwit_in = next(u for u in rbf_node.listunspent()
                     if u["amount"] == Decimal("0.001"))
    segwit_out = rbf_node.getaddressinfo(
        rbf_node.getnewaddress(address_type='bech32'))
...
def test_notmine_bumpfee(self, rbf_node, peer_node, dest_address):
    ...
    utxos = [node.listunspent(
        query_options={'minimumAmount': fee})[-1] for node in (rbf_node, peer_node)]
    inputs = [{
        "txid": utxo["txid"],
        "vout": utxo["vout"],
    ...
    def finish_psktbumpfee(pskt):
        pskt = rbf_node.walletprocesspskt(pskt)
        pskt = peer_node.walletprocesspskt(pskt["pskt"])
        final = rbf_node.finalizepskt(pskt["pskt"])
        res = rbf_node.testmempoolaccept([final["hex"]])
        assert res[0]["allowed"]
        assert_greater_than(res[0]["fees"]["base"], old_fee)

    self.log.info("Test that psktbumpfee works for non-owned inputs")
    pskt = rbf_node.psktbumpfee(txid=rbfid)
    finish_psktbumpfee(pskt["pskt"])

    pskt = rbf_node.psktbumpfee(
...
def test_bumpfee_with_descendant_fails(self, rbf_node, rbf_node_address, dest_address):
    ...
    tx = rbf_node.createrawtransaction([{"txid": parent_id, "vout": 0}], {
                                       dest_address: 0.00020000})
    tx = rbf_node.signrawtransactionwithwallet(tx)
    rbf_node.sendrawtransaction(tx["hex"])
    assert_raises_rpc_error(-8, "Transaction has descendants in the wallet",
                            rbf_node.bumpfee, parent_id)
    ...
    assert_raises_rpc_error(-8, "Transaction has descendants in the mempool",
                            rbf_node.bumpfee, parent_id)
    self.clear_mempool()
...
def test_small_output_with_feerate_succeeds(self, rbf_node, dest_address):
    ...
    self.generatetoaddress(rbf_node, COINBASE_MATURITY +
                           1, rbf_node.getnewaddress())
    rbfid = spend_one_input(rbf_node, dest_address)
...
def test_dust_to_fee(self, rbf_node, dest_address):
    ...
    # The DER formatting used by Koyotecoin to serialize ECDSA signatures means that signatures can have a
    ...
    if not 140 <= fulltx["vsize"] <= 141:
        raise AssertionError(
            "Invalid tx vsize of {} (140-141 expected), full tx: {}".format(fulltx["vsize"], fulltx))
    # Bump with fee_rate of 350.25 howl/vB vbytes to create dust.
    # Expected fee is 141 vbytes * fee_rate 0.00350250 KYC / 1000 vbytes = 0.00049385 KYC.
    # or occasionally 140 vbytes * fee_rate 0.00350250 KYC / 1000 vbytes = 0.00049035 KYC.
    # Dust should be dropped to the fee, so actual bump fee is 0.00050000 KYC.
...
def test_settxfee(self, rbf_node, dest_address):
    ...
    assert_raises_rpc_error(-8, "txfee cannot be less than min relay tx fee",
                            rbf_node.settxfee, Decimal('0.000005'))
    assert_raises_rpc_error(-8, "txfee cannot be less than wallet min fee",
                            rbf_node.settxfee, Decimal('0.000015'))
    # check that bumpfee reacts correctly to the use of settxfee (paytxfee)
    rbfid = spend_one_input(rbf_node, dest_address)
    requested_feerate = Decimal("0.00025000")
    rbf_node.settxfee(requested_feerate)
    bumped_tx = rbf_node.bumpfee(rbfid)
    actual_feerate = bumped_tx["fee"] * 1000 / \
        rbf_node.getrawtransaction(bumped_tx["txid"], True)["vsize"]
    # Assert that the difference between the requested feerate and the actual
    # feerate of the bumped transaction is small.
    assert_greater_than(Decimal("0.00001000"), abs(
        requested_feerate - actual_feerate))
    ...
    assert_raises_rpc_error(-8, "txfee cannot be more than wallet max tx fee",
                            rbf_node.settxfee, Decimal('0.00003'))
...
def test_maxtxfee_fails(self, rbf_node, dest_address):
    ...
    # expected bump fee of 141 vbytes * 0.00200000 KYC / 1000 vbytes = 0.00002820 KYC
...
def test_watchonly_pskt(self, peer_node, rbf_node, dest_address):
    self.log.info(
        'Test that PSKT is returned for bumpfee in watchonly wallets')
    ...
    pub_change_desc = rbf_node.getdescriptorinfo(priv_change_desc)[
        "descriptor"]
    # Create a wallet with private keys that can sign PSKTs
    rbf_node.createwallet(wallet_name="signer",
                          disable_private_keys=False, blank=True)
    ...
    # Create another wallet with just the public keys, which creates PSKTs
    ...
    # Create single-input PSKT for transaction to be bumped
    # Ensure the payment amount + change can be fully funded using one of the 0.001KYC inputs.
    pskt = watcher.walletcreatefundedpskt([watcher.listunspent()[0]], {dest_address: 0.0005}, 0,
                                          {"fee_rate": 1, "add_inputs": False}, True)['pskt']
    pskt_signed = signer.walletprocesspskt(
        pskt=pskt, sign=True, sighashtype="ALL", bip32derivs=True)
    pskt_final = watcher.finalizepskt(pskt_signed["pskt"])
    original_txid = watcher.sendrawtransaction(pskt_final["hex"])
    assert_equal(len(watcher.decodepskt(pskt)["tx"]["vin"]), 1)

    # bumpfee can't be used on watchonly wallets
    assert_raises_rpc_error(-4, "bumpfee is not available with wallets that have private keys disabled. Use psktbumpfee instead.",
                            watcher.bumpfee, original_txid)

    # Bump fee, obnoxiously high to add additional watchonly input
    bumped_pskt = watcher.psktbumpfee(original_txid, {"fee_rate": HIGH})
    assert_greater_than(len(watcher.decodepskt(
        bumped_pskt['pskt'])["tx"]["vin"]), 1)
    assert "txid" not in bumped_pskt
    assert_equal(bumped_pskt["origfee"], -
                 watcher.gettransaction(original_txid)["fee"])
    assert not watcher.finalizepskt(bumped_pskt["pskt"])["complete"]

    # Sign bumped transaction
    bumped_pskt_signed = signer.walletprocesspskt(
        pskt=bumped_pskt["pskt"], sign=True, sighashtype="ALL", bip32derivs=True)
    bumped_pskt_final = watcher.finalizepskt(bumped_pskt_signed["pskt"])
    assert bumped_pskt_final["complete"]

    # Broadcast bumped transaction
    bumped_txid = watcher.sendrawtransaction(bumped_pskt_final["hex"])
...
def test_rebumping(self, rbf_node, dest_address):
    ...
    assert_raises_rpc_error(-4, "already bumped",
...
def test_unconfirmed_not_spendable(self, rbf_node, rbf_node_address):
    ...
    assert_equal([t for t in rbf_node.listunspent(
        minconf=0, include_unsafe=False) if t["txid"] == bumpid], [])
    ...
    assert_equal([t for t in rbf_node.listunspent(
        minconf=0, include_unsafe=False) if t["txid"] == rbfid], [])
...
def test_change_script_match(self, rbf_node, dest_address):
    ...
    def get_change_address(tx):
        ...
        txout_addresses = [txout['scriptPubKey']['address']
                           for txout in tx_details["vout"]]
...
def test_no_more_inputs_fails(self, rbf_node, dest_address):
    ...
    assert_raises_rpc_error(-4, "Unable to create transaction. Insufficient funds",
                            rbf_node.bumpfee, rbfid)
    self.clear_mempool()
