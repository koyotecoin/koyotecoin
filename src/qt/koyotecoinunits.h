...
#ifndef KOYOTECOIN_QT_KOYOTECOINUNITS_H
#define KOYOTECOIN_QT_KOYOTECOINUNITS_H
...
/** Koyotecoin unit definitions. Encapsulates parsing and formatting
   and serves as list model for drop-down selection boxes.
*/
class KoyotecoinUnits: public QAbstractListModel
{
    Q_OBJECT
...
public:
    ...
    //! Number of Howloshis (1e-8) per unit
    ...
    //! Return maximum number of base units (Howloshis)
    static CAmount maxMoney();

private:
    QList<Unit> unitlist;
};
typedef KoyotecoinUnits::Unit KoyotecoinUnit;

QDataStream& operator<<(QDataStream& out, const KoyotecoinUnit& unit);
QDataStream& operator>>(QDataStream& in, KoyotecoinUnit& unit);

#endif // KOYOTECOIN_QT_KOYOTECOINUNITS_H
