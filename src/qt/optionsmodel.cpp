...
/** Call node.updateRwSetting() with Koyotecoin 22.x workaround. */
...
//! Convert enabled/size values to koyotecoin -prune setting.
...
//! Get pruning enabled value to show in GUI from koyotecoin -prune setting.
...
//! Get pruning size value to show in GUI from koyotecoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayKoyotecoinUnit")) {
        settings.setValue("DisplayKoyotecoinUnit", QVariant::fromValue(KoyotecoinUnit::KYC));
    }
    QVariant unit = settings.value("DisplayKoyotecoinUnit");
    if (unit.canConvert<KoyotecoinUnit>()) {
        m_display_koyotecoin_unit = unit.value<KoyotecoinUnit>();
    } else {
        m_display_koyotecoin_unit = KoyotecoinUnit::KYC;
        settings.setValue("DisplayKoyotecoinUnit", QVariant::fromValue(m_display_koyotecoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if koyotecoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_koyotecoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<KoyotecoinUnit>() == m_display_koyotecoin_unit) return;
    m_display_koyotecoin_unit = new_unit.value<KoyotecoinUnit>();
    QSettings settings;
    settings.setValue("DisplayKoyotecoinUnit", QVariant::fromValue(m_display_koyotecoin_unit));
    Q_EMIT displayUnitChanged(m_display_koyotecoin_unit);
}
