...
const std::string KoyotecoinGUI::DEFAULT_UIPLATFORM =
...
KoyotecoinGUI::KoyotecoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &KoyotecoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &KoyotecoinGUI::showModalOverlay);
...
void KoyotecoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Koyotecoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and koyotecoin: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &KoyotecoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &KoyotecoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &KoyotecoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    openAction->setStatusTip(tr("Open a koyotecoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Koyotecoin command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &KoyotecoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &KoyotecoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &KoyotecoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &KoyotecoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &KoyotecoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &KoyotecoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &KoyotecoinGUI::setCurrentWallet);
                    activity->open(path);
                });
            }
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &KoyotecoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &KoyotecoinGUI::setCurrentWallet);
            activity->create();
        });
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_mask_values_action, &QAction::toggled, this, &KoyotecoinGUI::setPrivacy);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &KoyotecoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &KoyotecoinGUI::showDebugWindow);
}

void KoyotecoinGUI::createMenuBar()
{
#ifdef Q_OS_MACOS
...
void KoyotecoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &KoyotecoinGUI::setCurrentWalletBySelectorIndex);
...
void KoyotecoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &KoyotecoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &KoyotecoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &KoyotecoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &KoyotecoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void KoyotecoinGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &KoyotecoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &KoyotecoinGUI::removeWallet);

    auto activity = new LoadWalletsActivity(m_wallet_controller, this);
    activity->load();
}

WalletController* KoyotecoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void KoyotecoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &KoyotecoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &KoyotecoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &KoyotecoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &KoyotecoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &KoyotecoinGUI::incomingTransaction);
    connect(this, &KoyotecoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
    wallet_view->setPrivacy(isPrivacyModeActivated());
    const QString display_name = walletModel->getDisplayName();
    m_wallet_selector->addItem(display_name, QVariant::fromValue(walletModel));
}

void KoyotecoinGUI::removeWallet(WalletModel* walletModel)
{
    if (!walletFrame) return;
...
void KoyotecoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame) return;
...
void KoyotecoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void KoyotecoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void KoyotecoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void KoyotecoinGUI::createTrayIcon()
{
...
void KoyotecoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &KoyotecoinGUI::toggleHidden);
...
void KoyotecoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void KoyotecoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void KoyotecoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void KoyotecoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void KoyotecoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void KoyotecoinGUI::openClicked()
{
...
void KoyotecoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void KoyotecoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void KoyotecoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void KoyotecoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void KoyotecoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void KoyotecoinGUI::gotoVerifyMessageTab(QString addr)
{
...
void KoyotecoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Koyotecoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void KoyotecoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void KoyotecoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void KoyotecoinGUI::updateHeadersSyncProgressLabel()
{
...
void KoyotecoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void KoyotecoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &KoyotecoinGUI::quitRequested);
...
void KoyotecoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void KoyotecoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void KoyotecoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &KoyotecoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &KoyotecoinGUI::show);
                e->ignore();
            }
        }
    }
...
void KoyotecoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void KoyotecoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void KoyotecoinGUI::incomingTransaction(const QString& date, KoyotecoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(KoyotecoinUnits::formatWithUnit(unit, amount, true));
...
void KoyotecoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void KoyotecoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool KoyotecoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool KoyotecoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void KoyotecoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void KoyotecoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void KoyotecoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void KoyotecoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void KoyotecoinGUI::updateWindowTitle()
{
...
void KoyotecoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void KoyotecoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void KoyotecoinGUI::detectShutdown()
{
...
void KoyotecoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void KoyotecoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(KoyotecoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = KoyotecoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void KoyotecoinGUI::subscribeToCoreSignals()
{
...
void KoyotecoinGUI::unsubscribeFromCoreSignals()
{
...
bool KoyotecoinGUI::isPrivacyModeActivated() const
{
      ...
      m_platform_style{platformStyle}
{
    ...
    QList<KoyotecoinUnit> units = KoyotecoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const KoyotecoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, KoyotecoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const KoyotecoinUnit u : KoyotecoinUnits::availableUnits()) {
        menu->addAction(KoyotecoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(KoyotecoinUnit newUnits)
{
    setText(KoyotecoinUnits::longName(newUnits));
}
