...
#ifndef KOYOTECOIN_PSKT_H
#define KOYOTECOIN_PSKT_H
...
// Magic bytes
static constexpr uint8_t PSKT_MAGIC_BYTES[5] = {'p', 's', 'k', 't', 0xff};

// Global types
static constexpr uint8_t PSKT_GLOBAL_UNSIGNED_TX = 0x00;
static constexpr uint8_t PSKT_GLOBAL_XPUB = 0x01;
static constexpr uint8_t PSKT_GLOBAL_VERSION = 0xFB;
static constexpr uint8_t PSKT_GLOBAL_PROPRIETARY = 0xFC;

// Input types
static constexpr uint8_t PSKT_IN_NON_WITNESS_UTXO = 0x00;
static constexpr uint8_t PSKT_IN_WITNESS_UTXO = 0x01;
static constexpr uint8_t PSKT_IN_PARTIAL_SIG = 0x02;
static constexpr uint8_t PSKT_IN_SIGHASH = 0x03;
static constexpr uint8_t PSKT_IN_REDEEMSCRIPT = 0x04;
static constexpr uint8_t PSKT_IN_WITNESSSCRIPT = 0x05;
static constexpr uint8_t PSKT_IN_BIP32_DERIVATION = 0x06;
static constexpr uint8_t PSKT_IN_SCRIPTSIG = 0x07;
static constexpr uint8_t PSKT_IN_SCRIPTWITNESS = 0x08;
static constexpr uint8_t PSKT_IN_RIPEMD160 = 0x0A;
static constexpr uint8_t PSKT_IN_SHA256 = 0x0B;
static constexpr uint8_t PSKT_IN_HASH160 = 0x0C;
static constexpr uint8_t PSKT_IN_HASH256 = 0x0D;
static constexpr uint8_t PSKT_IN_TAP_KEY_SIG = 0x13;
static constexpr uint8_t PSKT_IN_TAP_SCRIPT_SIG = 0x14;
static constexpr uint8_t PSKT_IN_TAP_LEAF_SCRIPT = 0x15;
static constexpr uint8_t PSKT_IN_TAP_BIP32_DERIVATION = 0x16;
static constexpr uint8_t PSKT_IN_TAP_INTERNAL_KEY = 0x17;
static constexpr uint8_t PSKT_IN_TAP_MERKLE_ROOT = 0x18;
static constexpr uint8_t PSKT_IN_PROPRIETARY = 0xFC;

// Output types
static constexpr uint8_t PSKT_OUT_REDEEMSCRIPT = 0x00;
static constexpr uint8_t PSKT_OUT_WITNESSSCRIPT = 0x01;
static constexpr uint8_t PSKT_OUT_BIP32_DERIVATION = 0x02;
static constexpr uint8_t PSKT_OUT_TAP_INTERNAL_KEY = 0x05;
static constexpr uint8_t PSKT_OUT_TAP_TREE = 0x06;
static constexpr uint8_t PSKT_OUT_TAP_BIP32_DERIVATION = 0x07;
static constexpr uint8_t PSKT_OUT_PROPRIETARY = 0xFC;

// The separator is 0x00. Reading this in means that the unserializer can interpret it
// as a 0 length key which indicates that this is the separator. The separator has no value.
static constexpr uint8_t PSKT_SEPARATOR = 0x00;

// BIP 174 does not specify a maximum file size, but we set a limit anyway
// to prevent reading a stream indefinitely and running out of memory.
const std::streamsize MAX_FILE_SIZE_PSKT = 100000000; // 100 MB

// PSKT version number
static constexpr uint32_t PSKT_HIGHEST_VERSION = 0;

/** A structure for PSKT proprietary types */
struct PSKTProprietary
{
    uint64_t subtype;
    ...
    bool operator<(const PSKTProprietary &b) const {
        return key < b.key;
    }
    bool operator==(const PSKTProprietary &b) const {
        return key == b.key;
...
/** A structure for PSKTs which contain per-input information */
struct PSKTInput
{
    ...
    std::set<PSKTProprietary> m_proprietary;
    std::optional<int> sighash_type;

    bool IsNull() const;
    void FillSignatureData(SignatureData& sigdata) const;
    void FromSignatureData(const SignatureData& sigdata);
    void Merge(const PSKTInput& input);
    PSKTInput() {}
    ...
    inline void Serialize(Stream& s) const {
        // Write the utxo
        if (non_witness_utxo) {
            SerializeToVector(s, CompactSizeWriter(PSKT_IN_NON_WITNESS_UTXO));
            OverrideStream<Stream> os(&s, s.GetType(), s.GetVersion() | SERIALIZE_TRANSACTION_NO_WITNESS);
            SerializeToVector(os, non_witness_utxo);
        }
        if (!witness_utxo.IsNull()) {
            SerializeToVector(s, CompactSizeWriter(PSKT_IN_WITNESS_UTXO));
        ...
        if (final_script_sig.empty() && final_script_witness.IsNull()) {
            ...
            for (auto sig_pair : partial_sigs) {
                SerializeToVector(s, CompactSizeWriter(PSKT_IN_PARTIAL_SIG), Span{sig_pair.second.first});
                s << sig_pair.second.second;
            }

            // Write the sighash type
            if (sighash_type != std::nullopt) {
                SerializeToVector(s, CompactSizeWriter(PSKT_IN_SIGHASH));
                SerializeToVector(s, *sighash_type);
            }

            // Write the redeem script
            if (!redeem_script.empty()) {
                SerializeToVector(s, CompactSizeWriter(PSKT_IN_REDEEMSCRIPT));
                s << redeem_script;
            }

            // Write the witness script
            if (!witness_script.empty()) {
                SerializeToVector(s, CompactSizeWriter(PSKT_IN_WITNESSSCRIPT));
                s << witness_script;
            }

            // Write any hd keypaths
            SerializeHDKeypaths(s, hd_keypaths, CompactSizeWriter(PSKT_IN_BIP32_DERIVATION));

            // Write any ripemd160 preimage
            for (const auto& [hash, preimage] : ripemd160_preimages) {
                SerializeToVector(s, CompactSizeWriter(PSKT_IN_RIPEMD160), Span{hash});
                s << preimage;
            }

            // Write any sha256 preimage
            for (const auto& [hash, preimage] : sha256_preimages) {
                SerializeToVector(s, CompactSizeWriter(PSKT_IN_SHA256), Span{hash});
                s << preimage;
            }

            // Write any hash160 preimage
            for (const auto& [hash, preimage] : hash160_preimages) {
                SerializeToVector(s, CompactSizeWriter(PSKT_IN_HASH160), Span{hash});
                s << preimage;
            }

            // Write any hash256 preimage
            for (const auto& [hash, preimage] : hash256_preimages) {
                SerializeToVector(s, CompactSizeWriter(PSKT_IN_HASH256), Span{hash});
                s << preimage;
            }

            // Write taproot key sig
            if (!m_tap_key_sig.empty()) {
                SerializeToVector(s, PSKT_IN_TAP_KEY_SIG);
                s << m_tap_key_sig;
            }
            ...
            for (const auto& [pubkey_leaf, sig] : m_tap_script_sigs) {
                ...
                SerializeToVector(s, PSKT_IN_TAP_SCRIPT_SIG, xonly, leaf_hash);
                s << sig;
            }
            ...
            for (const auto& [leaf, control_blocks] : m_tap_scripts) {
                ...
                for (const auto& control_block : control_blocks) {
                    SerializeToVector(s, PSKT_IN_TAP_LEAF_SCRIPT, Span{control_block});
            ...
            for (const auto& [xonly, leaf_origin] : m_tap_bip32_paths) {
                ...
                SerializeToVector(s, PSKT_IN_TAP_BIP32_DERIVATION, xonly);
                std::vector<unsigned char> value;
            ...
            if (!m_tap_internal_key.IsNull()) {
                SerializeToVector(s, PSKT_IN_TAP_INTERNAL_KEY);
                s << ToByteVector(m_tap_internal_key);
            }

            // Write taproot merkle root
            if (!m_tap_merkle_root.IsNull()) {
                SerializeToVector(s, PSKT_IN_TAP_MERKLE_ROOT);
        ...
        if (!final_script_sig.empty()) {
            SerializeToVector(s, CompactSizeWriter(PSKT_IN_SCRIPTSIG));
            s << final_script_sig;
        }
        // write script witness
        if (!final_script_witness.IsNull()) {
            SerializeToVector(s, CompactSizeWriter(PSKT_IN_SCRIPTWITNESS));
    ...
    inline void Unserialize(Stream& s) {
        ...
        while(!s.empty()) {
            ...
            switch(type) {
                ...
                case PSKT_IN_BIP32_DERIVATION:
                {
                    DeserializeHDKeypaths(s, key, hd_keypaths);
                    break;
                }
                case PSKT_IN_SCRIPTSIG:
                {
                    if (!key_lookup.emplace(key).second) {
                ...
                case PSKT_IN_TAP_BIP32_DERIVATION:
                {
                    if (!key_lookup.emplace(key).second) {
                ...
                case PSKT_IN_PROPRIETARY:
                {
                    PSKTProprietary this_prop;
    ...
    template <typename Stream>
    PSKTInput(deserialize_type, Stream& s) {
        Unserialize(s);
    }
};

/** A structure for PSKTs which contains per output information */
struct PSKTOutput
{
    CScript redeem_script;
    CScript witness_script;
    ...
    void FillSignatureData(SignatureData& sigdata) const;
    void FromSignatureData(const SignatureData& sigdata);
    void Merge(const PSKTOutput& output);
    PSKTOutput() {}
    ...
    inline void Serialize(Stream& s) const {
        // Write the redeem script
        if (!redeem_script.empty()) {
            SerializeToVector(s, CompactSizeWriter(PSKT_OUT_REDEEMSCRIPT));
            s << redeem_script;
        }

        // Write the witness script
        if (!witness_script.empty()) {
            SerializeToVector(s, CompactSizeWriter(PSKT_OUT_WITNESSSCRIPT));
            s << witness_script;
        }

        // Write any hd keypaths
        SerializeHDKeypaths(s, hd_keypaths, CompactSizeWriter(PSKT_OUT_BIP32_DERIVATION));
        ...
        // Write taproot internal key
        if (!m_tap_internal_key.IsNull()) {
            SerializeToVector(s, PSKT_OUT_TAP_INTERNAL_KEY);
            s << ToByteVector(m_tap_internal_key);
        }

        // Write taproot tree
        if (!m_tap_tree.empty()) {
            SerializeToVector(s, PSKT_OUT_TAP_TREE);
            std::vector<unsigned char> value;
            CVectorWriter s_value(s.GetType(), s.GetVersion(), value, 0);
        ...
        for (const auto& [xonly, leaf] : m_tap_bip32_paths) {
            ...
            SerializeToVector(s, PSKT_OUT_TAP_BIP32_DERIVATION, xonly);
            std::vector<unsigned char> value;
            CVectorWriter s_value(s.GetType(), s.GetVersion(), value, 0);
            s_value << leaf_hashes;
            SerializeKeyOrigin(s_value, origin);
    ...
    inline void Unserialize(Stream& s) {
        ...
        while(!s.empty()) {
            ...
            switch(type) {
                ...
                case PSKT_OUT_BIP32_DERIVATION:
                {
                    DeserializeHDKeypaths(s, key, hd_keypaths);
                    break;
                }
                case PSKT_OUT_TAP_INTERNAL_KEY:
                {
                    if (!key_lookup.emplace(key).second) {
                ...
                case PSKT_OUT_TAP_BIP32_DERIVATION:
                {
                    if (!key_lookup.emplace(key).second) {
    ...
    template <typename Stream>
    PSKTOutput(deserialize_type, Stream& s) {
        Unserialize(s);
    }
};

/** A version of CTransaction with the PSKT format*/
struct PartiallySignedTransaction
{
    ...
    std::vector<PSKTInput> inputs;
    std::vector<PSKTOutput> outputs;
    std::map<std::vector<unsigned char>, std::vector<unsigned char>> unknown;
    ...
    /** Merge pskt into this. The two pskts must have the same underlying CTransaction (i.e. the
      * same actual Koyotecoin transaction.) Returns true if the merge succeeded, false otherwise. */
    [[nodiscard]] bool Merge(const PartiallySignedTransaction& pskt);
    bool AddInput(const CTxIn& txin, PSKTInput& psktin);
    bool AddOutput(const CTxOut& txout, const PSKTOutput& psktout);
    ...
    inline void Serialize(Stream& s) const {
        ...
        // unsigned tx flag
        SerializeToVector(s, CompactSizeWriter(PSKT_GLOBAL_UNSIGNED_TX));
        ...
        for (const auto& xpub_pair : m_xpubs) {
            for (const auto& xpub : xpub_pair.second) {
                ...
                SerializeToVector(s, PSKT_GLOBAL_XPUB, ser_xpub);
        ...
        // PSKT version
        if (GetVersion() > 0) {
            SerializeToVector(s, CompactSizeWriter(PSKT_GLOBAL_VERSION));
        ...
        // Write inputs
        for (const PSKTInput& input : inputs) {
            s << input;
        }
        // Write outputs
        for (const PSKTOutput& output : outputs) {
            s << output;
        }
    }
    ...
    inline void Unserialize(Stream& s) {
        ...
        uint8_t magic[5];
        s >> magic;
        if (!std::equal(magic, magic + 5, PSKT_MAGIC_BYTES)) {
            throw std::ios_base::failure("Invalid PSKT magic bytes");
        }
        ...
        while(!s.empty()) {
            ...
            switch(type) {
                ...
                case PSKT_GLOBAL_VERSION:
                {
                    ...
                    m_version = v;
                    if (*m_version > PSKT_HIGHEST_VERSION) {
                        throw std::ios_base::failure("Unsupported version number");
                    }
                    break;
                }
                case PSKT_GLOBAL_PROPRIETARY:
                {
                    PSKTProprietary this_prop;
                    skey >> this_prop.identifier;
                    this_prop.subtype = ReadCompactSize(skey);
...
/** Compute a PrecomputedTransactionData object from a pskt. */
PrecomputedTransactionData PrecomputePSKTData(const PartiallySignedTransaction& pskt);

/** Checks whether a PSKTInput is already signed. */
bool PSKTInputSigned(const PSKTInput& input);

/** Signs a PSKTInput, verifying that all provided data matches what is being signed.
 *
 * txdata should be the output of PrecomputePSKTData (which can be shared across
 * multiple SignPSKTInput calls). If it is nullptr, a dummy signature will be created.
 **/
bool SignPSKTInput(const SigningProvider& provider, PartiallySignedTransaction& pskt, int index, const PrecomputedTransactionData* txdata, int sighash = SIGHASH_ALL, SignatureData* out_sigdata = nullptr, bool finalize = true);

/** Counts the unsigned inputs of a PSKT. */
size_t CountPSKTUnsignedInputs(const PartiallySignedTransaction& pskt);

/** Updates a PSKTOutput with information from provider.
 *
...
/**
 * Finalizes a PSKT if possible, combining partial signatures.
 *
 * @param[in,out] psktx PartiallySignedTransaction to finalize
 * return True if the PSKT is now complete, false otherwise
 */
bool FinalizePSKT(PartiallySignedTransaction& psktx);

/**
 * Finalizes a PSKT if possible, and extracts it to a CMutableTransaction if it could be finalized.
 *
 * @param[in]  psktx PartiallySignedTransaction
...
/**
 * Combines PSKTs with the same underlying transaction, resulting in a single PSKT with all partial signatures from each input.
 *
 * @param[out] out   the combined PSKT, if successful
 * @param[in]  psktxs the PSKTs to combine
 * @return error (OK if we successfully combined the transactions, other error if they were not compatible)
 */
[[nodiscard]] TransactionError CombinePSKTs(PartiallySignedTransaction& out, const std::vector<PartiallySignedTransaction>& psktxs);

//! Decode a base64ed PSKT into a PartiallySignedTransaction
[[nodiscard]] bool DecodeBase64PSKT(PartiallySignedTransaction& decoded_pskt, const std::string& base64_pskt, std::string& error);
//! Decode a raw (binary blob) PSKT into a PartiallySignedTransaction
[[nodiscard]] bool DecodeRawPSKT(PartiallySignedTransaction& decoded_pskt, Span<const std::byte> raw_pskt, std::string& error);

#endif // KOYOTECOIN_PSKT_H
