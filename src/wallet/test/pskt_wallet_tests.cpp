...
namespace wallet {
BOOST_FIXTURE_TEST_SUITE(pskt_wallet_tests, WalletTestingSetup)
...
BOOST_AUTO_TEST_CASE(pskt_updater_test)
{
    ...
    bool complete = true;
    BOOST_REQUIRE_EQUAL(TransactionError::OK, m_wallet.FillPSKT(psktx, complete, SIGHASH_ALL, false, true));
    ...
    psktx.tx->vin[0].prevout.n = 2;

    // Try to sign the mutated input
    SignatureData sigdata;
    BOOST_CHECK(m_wallet.FillPSKT(psktx, complete, SIGHASH_ALL, true, true) != TransactionError::OK);
}
