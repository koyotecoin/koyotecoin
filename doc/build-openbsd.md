...
This guide describes how to build koyotecoind, command-line utilities, and GUI on OpenBSD.
...
### 2. Clone Koyotecoin Repo
Clone the Koyotecoin Core repository to a directory. All build scripts and commands will run from this directory.
``` bash
...
It is not necessary to build wallet functionality to run either `koyotecoind` or `koyotecoin-qt`.
...
Koyotecoin Core includes a GUI built with the cross-platform Qt Framework. To compile the GUI, Qt 5 is required.
